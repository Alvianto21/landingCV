@props(['id', 'name', 'value', 'default', 'error', 'alter'])
<label for="{{ $id }}" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{ $slot }}</label>
@if (! empty($default))
	<img class="img-preview img-fluid img-thumbnail items-center mb-3" width="200" src="{{ asset('storage/' . $default) }}" alt="{{ $alter }}">
@endif
<input type="file" name="{{ $name }}" id="{{ $id }}" class="cursor-pointer bg-neutral-secondary-medium bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" value="{{ old($value, $default) }}" {{ $attributes }} accept=".jpg, .jpeg, .png, .svg" autocomplete="photo">
